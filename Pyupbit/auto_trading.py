...
append_lists = []
remove_lists = ["KRW-SNT", "KRW-XRP", "KRW-SRN"]

# access_key : 업비트에서 받은 본인의 API Access Key
# secret_key : 업비트에서 받은 본인의 API Secret Key
# server_url : 업비트 서버 주소, 'https://api.upbit.com'으로 고정

# acc_trade_price_24h : 기준이 되는 거래대금, 해당 거래대금 이하의 상품들은 분석하지 않음 
# append_lists : 개별적으로 추가하고 싶은 코인. 리스트 형태로 입력
# remove_lists : 개별적으로 삭제하고 싶은 코인. 리스트 형태로 입력
...
while True:
    ...
    if now.strftime("%M") == '30' or now.strftime("%M") == '00':  # 매시 정각, 30분 마다 분석 후 매매
        ...
        # krw_coins_final = [i for i in krw_coins if ticker_info(i)[0]['acc_trade_price_24h'] > acc_trade_price_24h]
                    ...
                    if pre_MA4 < pre_MA48:  # 차트의 흐름이 상승장 초입이라고 판단                           
                        for bid_coin in krw_coins_final:  # 사용자가 매수를 원하는 코인 일치 여부 확인
                            if coin == bid_coin:
                                try:
                                    print(f"!!! {bid_coin} 매수 !!!")
                                    bid_market(bid_coin, None, '50000')
                                except Exception as e:
